...
if(!$purchase_filename ||
   !$payment_id){
    printf "cancel_purchase.pl PURCHASE_FILENAME PAYMENT_ID\n";
...
my $cancel_purchase_url = 'http://' . $host . '/purchase/cancel';
...
while(!$success && $i < 5){
    ...
    DEBUG "cancel purchase - payment id: " . $payment_id;

    my $purchase = Monoidea::HTTP::Action::Purchase->new(url => $cancel_purchase_url,
							 payment_id => $payment_id);
    my $response = $purchase->cancel_purchase($user_agent);
	...
	DEBUG "cancel purchase - success ";

	$node = XML::LibXML::Element->new('status');
	$node->setAttribute(value => 'cancel');
    ...
    if(!$success){
	DEBUG "cancel purchase failed - attempt " . ($i + 1) . "/5 retrying in 1 seconds";
...
if(!$success){
	...
	ERROR "cancel purchase failed - giving up";

	$node = XML::LibXML::Element->new('status');
	$node->setAttribute(value => 'error: failed to cancel');
	$purchase_node->appendChild($node);
}
