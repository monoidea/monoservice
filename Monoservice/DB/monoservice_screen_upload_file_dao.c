					  ...
					  gboolean available)
{
  ...
  query = g_strdup_printf("INSERT INTO SCREEN_UPLOAD_FILE (FILENAME, CREATION_TIME, DURATION, AVAILABLE) VALUES (%s, %ld, %f, %s)",
			  filename,
			  creation_time,
			  duration_value,
...
void
monoservice_screen_upload_file_dao_set_creation_time(MonoserviceMysqlConnector *mysql_connector,
						     guint64 screen_upload_file_id,
						     time_t creation_time)
{
  gchar *query;
  ...
  query = g_strdup_printf("UPDATE SCREEN_UPLOAD_FILE SET CREATION_TIME = %ld WHERE SCREEN_UPLOAD_FILE_ID = '%lu'", creation_time, screen_upload_file_id);
