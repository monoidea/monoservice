				       ...
				       gboolean available)
{
  ...
  query = g_strdup_printf("INSERT INTO MIC_UPLOAD_FILE (FILENAME, CREATION_TIME, DURATION, AVAILABLE) VALUES (%s, %ld, %f, %s)",
			  filename,
			  creation_time,
			  duration_value,
...
void
monoservice_mic_upload_file_dao_set_creation_time(MonoserviceMysqlConnector *mysql_connector,
						  guint64 mic_upload_file_id,
						  time_t creation_time)
{
  gchar *query;
  ...
  query = g_strdup_printf("UPDATE MIC_UPLOAD_FILE SET CREATION_TIME = %ld WHERE MIC_UPLOAD_FILE_ID = '%lu'", creation_time, mic_upload_file_id);
